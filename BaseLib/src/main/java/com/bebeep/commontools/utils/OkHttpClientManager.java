...
//				.build();
	...
	private void _downloadAsyn(final String url, final String destFileDir,
		...
		call.enqueue(new Callback() {
			...
			public void onFailure(final Request request, final IOException e) {
				sendFailedStringCallback(request, e, 0,callback);
			}
			...
			public void onResponse(Response response) {
				...
				try {
					...
					sendFailedStringCallback(response.request(), e,0, callback);
				} finally {
					try {
						if (is != null)
							is.close();
	...
	private void deliveryResult(final ResultCallback callback, Request request) {
		mOkHttpClient.newCall(request).enqueue(new Callback() {
			...
			public void onResponse(final Response response) {
				...
				{
					sendFailedStringCallback(response.request(), e, 1,callback);
				}
	...
	private void sendFailedStringCallback(final Request request,
										  final Exception e, final int code, final ResultCallback callback) {
		mDelivery.post(new Runnable() {
			@Override
			public void run() {
				if (callback != null)
